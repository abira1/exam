...
export function ParagraphGapQuestion({
  ...
  // Split paragraph by question number patterns like (6).......... or 6…………………… or (6)……………………
  const renderParagraphWithGaps = () => {
    // Match both formats: (14)…… and 14……
    const parts = paragraph.split(/(\(?\d+\)?\s*[.…]+)/);
    
    return parts.map((part, idx) => {
      // Check if this part matches the pattern (number).... or number.... or (number)……………………
      ...
      if (match) {
        ...
        // Only render input if this questionNumber is in the questionNumbers array
